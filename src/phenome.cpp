...
namespace HyperNEAT {
    Phenome::Phenome(Genome &genome,
        ...
        for (auto &input : _inputs) {
            ...
            _nodes[_node_count] = {calculate_weight(input, input),
                                   NodeType::Input,
                                   nullptr};
            _node_count++;
        }
        for (auto &output : _outputs) {
            ...
            _nodes[_node_count] = {calculate_weight(output, output),
                                   NodeType::Output,
                                   _params.output_activation};
            _node_count++;
        }
    ...
    void Phenome::prune_extract(
        ...
        if (_pointset.count(point) == 0) {
            ...
            _nodes[_node_count] = {calculate_weight(point, point),
                                   NodeType::Hidden,
                                   _params.hidden_activation};
            _node_count++;
        }
        ...
        for (auto &child : Quadtree->children) {
            ...
            if (child->get_variance() > _params.variance_threshold) {
                ...
                if (outgoing) {
                    ...
                    d_left = std::fabs(
                        child->weight -
                        calculate_weight({center.x - child->size, center.y},
                                         point));
                    d_right = std::fabs(
                        child->weight -
                        calculate_weight({center.x + child->size, center.y},
                    ...
                    d_top = std::fabs(
                        child->weight -
                        calculate_weight({center.x, center.y - child->size},
                                         point));
                    d_bottom = std::fabs(
                        child->weight -
                        calculate_weight({center.x, center.y + child->size},
                ...
                if (band > _params.band_threshold) {
                    ...
                    if (_nodes[edge.to].type != NodeType::Input) {
                        connections[edge] =
                            child->weight * _params.weight_range;
                    }
                }
            }
        }
    ...
    void Phenome::cleanup() {
        ...
        for (auto &e : _edges) {
            if (_nodes.count(e.first.from) == 0 ||
                _nodes.count(e.first.to) == 0 || e.second == 0.0) {
        ...
        if (_edges.size() == 0) {
            ...
            for (auto &input : _inputs) {
                for (auto &output : _outputs) {
                    ...
                    _edges[{i, j}] =
                        calculate_weight(input, output) * _params.weight_range;
                }
            }
        }
        generate_adjacency();
    ...
    double Phenome::get_fitness() { return _genome.get_fitness(); }

    void Phenome::set_fitness(double fitness) { _genome.set_fitness(fitness); }

    Genome &Phenome::get_genome() { return _genome; }
} // namespace HyperNEAT